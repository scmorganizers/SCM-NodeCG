<template>
	<div class="Flex" :style="{ 'font-size': '3em', position: 'fixed' }">
		<img src="../stc.png" :style="{ 'margin-right': '10px' }" />
		<div id="Total">
			<span
...
<script lang="ts">
	...
	import { DonationTotal as DonationTotalType } from '@src/types/schemas';
	import { replicantNS } from '@src/browser_shared/replicant_store';

	@Component
	export default class DonationTotal extends Vue {
		@replicantNS.State((s) => s.reps.donationTotal)
		readonly donationTotal!: DonationTotalType;
		tweened = 0;
		created(): void {
		...
		get total(): string {
			return `$${this.tweened.toLocaleString('en-US', {
