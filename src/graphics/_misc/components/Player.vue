<template>
	...
	<div :style="{ position: 'fixed', transform: 'scale(0.95)' }">
		<div v-if="name" :key="name" class="Flex" :style="{ position: 'absolute', 'justify-content': align }">
			<div>
				<p v-if="!nameOnly" :style="{ 'font-family': 'Segoe UI Emoji', 'font-size': '1.25em', 'margin': 0, 'margin-top': '-0.5em' }">
					RUNNER <template v-if="hasMultipleTeams">{{ team }}</template>
				</p>
				
				<div ref="player" :style="{ 'font-size': '2.5em' }">
					{{ name }}
					<span v-if="finishTime" :style="{ 'font-size': '1em' }">
						[{{ finishTime }}]
					</span>
				</div>
			</div>
...
<script lang="ts">
	...
	import { RunDataActiveRun, Timer } from '@bundles/nodecg-speedcontrol/src/types';
	...
	export default class Player extends Vue {
		...
		@Prop({ default: 'center' }) readonly align!: string;
		timeout?: number;
		...
		onRunChange(val: RunDataActiveRun): void {
			...
			this.index = 0;
			this.name = null;
			const coop = !!(val?.teams?.length === 1 && val.teams[0].players.length >= 2);

			if (val) {
				if (coop && val.teams[0].players[this.teamI]) {
					this.name = val.teams[0].players[this.teamI].name;
				} else if (!coop && val.teams[this.teamI]?.players?.length) {
					this.showNextName();
				}
			}
